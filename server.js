...
function updateBook(req, res, next) {
  db.oneOrNone('UPDATE Books SET title = ${title}, author = ${author}, isbn = ${isbn}, courseName = ${coursename}, userID = ${userid}, price = ${price}, sold_date = ${sold_date} WHERE ID=${ID} RETURNING id', req.body)
    .then((data) => {
      returnDataOr404(res, data);
    })
    .catch((err) => {
