...
function updateUser(req, res, next) {
    db.oneOrNone('UPDATE Users SET emailAddress=${emailAddress}, name=${name}, username=${username}, passwordHash = ${passwordHash} WHERE ID=${ID} RETURNING id', req.body)
        .then(data => {
            returnDataOr404(res, data);
        })
        .catch(err => {
...
function createBook(req, res, next) {
    db.one('INSERT INTO Books(ID, title, author, isbn, courseName) VALUES (${ID}, ${title}, ${author}, ${isbn}, ${coursename}) RETURNING id', req.body)
        .then(data => {
            res.send(data);
        })
        .catch(err => {
